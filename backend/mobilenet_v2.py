...
class MobileNetV2:
    def __init__(self, num_classes, batch_size=32):
        ...
        self.result = dense(flatten(self.embedding), num_classes, name='result')


def conv2d(x, kernel, num_filter, strides, bn=True, act=tf.identity, name='conv', padding='SAME', is_train=True):
    ...
    with tf.device(DEVICE):
        with tf.variable_scope(name):
            ...
            conv = tf.nn.conv2d(x, w, strides, padding, use_cudnn_on_gpu=CUDNN_ON_GPU)
...
def depthwise_conv2d(x, kernel, strides, bn=True, act=tf.identity, name='conv_dw', is_train=True):
...
def fc(x, kernel, num_filter, strides, name='fc', padding='SAME', is_train=True):
    pre_channel = int(x.get_shape()[-1])
    shape = [kernel[0], kernel[1], pre_channel, num_filter]
    with tf.device(DEVICE):
        with tf.variable_scope(name):
